...
describe("canonical message persistence", () => {
  test("deduplicates repeated and cross-account deliveries within a tenant", async () => {
    ...
    const [created] = await store.reconcileMailbox({
      ...
      mailbox: first.location.mailbox, observations: [first],
    });
    ...
    const [repeated] = await store.reconcileMailbox({
      ...
      mailbox: duplicate.location.mailbox, observations: [duplicate],
    });
  ...
  test("keeps identities tenant-scoped", async () => {
    ...
    const [a] = await store.reconcileMailbox({ tenantId: "tenant-a", accountId: "imap-account", provider: "imap", mailbox: "INBOX", observations: [first] });
    const [b] = await store.reconcileMailbox({ tenantId: "tenant-b", accountId: "imap-account", provider: "imap", mailbox: "INBOX", observations: [second] });
  ...
  test("retains overlapping folders and reconciles moves without changing identity", async () => {
    ...
    const [canonical] = await store.reconcileMailbox({ tenantId: "tenant-a", accountId: "imap-account", provider: "imap", mailbox: "INBOX", observations: [inbox] });
    const archive = observation({ location: { ...inbox.location, mailbox: "Archive", uid: 99, modseq: "2" } });
    const [moved] = await store.reconcileMailbox({ tenantId: "tenant-a", accountId: "imap-account", provider: "imap", mailbox: "Archive", observations: [archive] });
    expect(await store.listMessageLocations("tenant-a", canonical!.id)).toHaveLength(2);

    await store.reconcileMailbox({ tenantId: "tenant-a", accountId: "imap-account", provider: "imap", mailbox: "INBOX", observations: [] });
  ...
  test("updates mutable flags on the location without changing message identity", async () => {
    ...
    const [first] = await store.reconcileMailbox({ tenantId: "tenant-a", accountId: "imap-account", provider: "imap", mailbox: "INBOX", observations: [unread] });
    const changed = observation({ location: { ...unread.location, modseq: "2", read: true, flagged: true } });
    const [second] = await store.reconcileMailbox({ tenantId: "tenant-a", accountId: "imap-account", provider: "imap", mailbox: "INBOX", observations: [changed] });
  ...
  test("uses stable provider location identity when Message-ID is invalid", async () => {
    ...
    const [first] = await store.reconcileMailbox({ tenantId: "tenant-a", accountId: "imap-account", provider: "imap", mailbox: "INBOX", observations: [missing] });
    const [second] = await store.reconcileMailbox({ tenantId: "tenant-a", accountId: "imap-account", provider: "imap", mailbox: "INBOX", observations: [{ ...missing, location: { ...missing.location, modseq: "2" } }] });
    expect(second!.id).toBe(first!.id);
  });
  ...
  test("does not collapse equal IMAP UIDs from different mailboxes without Message-ID", async () => {
    ...
    const [first] = await store.reconcileMailbox({ tenantId: "tenant-a", accountId: "imap-account", provider: "imap", mailbox: "INBOX", observations: [inbox] });
    const [second] = await store.reconcileMailbox({ tenantId: "tenant-a", accountId: "imap-account", provider: "imap", mailbox: "Archive", observations: [archive] });
