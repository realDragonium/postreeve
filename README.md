...
This is an early hackathon project built for the OpenAI WebMCP Challenge. It focuses on one complete workflow: browse, propose, review, approve, apply, and undo.
...
Open [http://127.0.0.1:3000](http://127.0.0.1:3000). The first run includes a fixture mailbox, so you can test the complete interface without email credentials.
...
Select **Add account** and enter the IMAP and SMTP settings supplied by your email provider. Prefer an app-specific password when the provider supports one. OAuth is not implemented yet.
